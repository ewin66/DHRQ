...
namespace Landi.FrameWorks
{
    public class ActivityManager
    {
        ...
        private string mConfigFileName; //界面配置文件名称 
        private ActivityManagerHandler mHandler;
        private string mStartupPath;

        private HandlerThread mManagerThread;//管理线程
        private HandlerThread mWorkThread;//工作线程，异步
        ...
        void mWebBrowser_Navigating(object sender, WebBrowserNavigatingEventArgs e)
        {
        ...
        {
            //AppLog.Write("DocumentCompleted URL:" + e.Url.ToString(), AppLog.LogMessageType.Info);
            if (mWebBrowser.ReadyState == WebBrowserReadyState.Complete && mWebBrowser.Document.Url.ToString().Trim() == e.Url.ToString().Trim())
            {
                onSetContentViewComplete();
            }
            //if (mWebBrowser.ReadyState < WebBrowserReadyState.Complete || mWebBrowser.Url.ToString() == _lastUrl) return;//加载的状态
            //_lastUrl = mWebBrowser.Url.ToString();
            //onSetContentViewComplete();
        }
        ...
        internal void SetContentView()
        {
            ...
            if (mPendingIntent.AlonePage) //整页加载
            {
                mWebBrowser.Invoke((MethodInvoker)delegate()
                {
        ...
        internal void Enter()
        {
            //Log.Info("Enter lpp" + currentActivity);
    ...
    public class TimerConfig
    {
        ...
        public override string ToString()
        {
            ...
            sb.Append("<span id=");
            sb.Append(Id);
            sb.Append(" style=\"position: absolute; width: ");
            sb.Append(Width);
            sb.Append("px; height: ");
            sb.Append(Height);
            sb.Append("px; top: ");
            ...
            sb.Append(";font-size:");
            sb.Append(Font_Size);
            sb.Append("px;text-align:center\"></span>");
            return sb.ToString();
        }
    }
}
