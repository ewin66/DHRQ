...
namespace Landi.FrameWorks
{
    public class Activity
    {
        ...
        protected static TransResult LastTransResult = TransResult.E_SEND_FAIL;
        ...
        protected static TransResult SyncTransaction(PackageBase package)
        {
            ...
            while (true)
            {
                lock (mLock)
                {
                    if (mTransCompleted)
                    {
                        mTransCompleted = false;
                        Application.DoEvents();
                        return LastTransResult;
                    }
                }
                Application.DoEvents();
            }
        ...
        protected static bool SetAutoRunCtrlRegInfo(bool isApp)
        {
            ...
            if (isApp)
                reg.SetValue("Payment.exe", Application.ExecutablePath);
            else
            {
        ...
        protected void setComponnents(string comName, bool comFlag, string homeName, bool retFlag, string backName, bool backFlag)
        {
            if (string.IsNullOrEmpty(comName) || string.IsNullOrEmpty(homeName) || string.IsNullOrEmpty(backName))
            {
            ...
            else
            {
                if (comFlag)
                {
                    ...
                    if (retFlag)
                    {
                        GetElementById(homeName).Style = "visibility: block;";
                    }
                    else
                    {
                        GetElementById(homeName).Style = "visibility: hidden;";
                    }
                    if (backFlag)
                    {
                        GetElementById(backName).Style = "visibility: block;";
                    }
                    else
                    {
                        GetElementById(backName).Style = "visibility: hidden;";
                    }
                }
                else
                {
