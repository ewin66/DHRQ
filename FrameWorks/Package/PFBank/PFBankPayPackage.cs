...
{
    public abstract class PFBankPayPackage : PackageBase
    {
        ...
        protected byte[] PackBagHead(int datalen)
        {
            int len = 0;
            string lenstr = "";
            string customerNo = "";
            ...
            len = customerNo.Length + datalen;
            lenstr = len.ToString().PadLeft(4, '\0');
            head = lenstr + customerNo;

            byte[] bhead = Encoding.GetEncoding("GBK").GetBytes(head);
        ...
        internal override TransResult transact()
        {
            ...
            try
            {
                ret = Packetflag ? PackBankBag() : PackJson();
                ...
                ret = Packetflag ? UnPackBankBag() : UnpackJson();
                if (ret != TransResult.E_SUCC)
                {
                    return ret;
                }
        ...
        private bool Unpackbagfix()
        {
            bool ret = false;
            string[] recvDatas = RecvBagData.Split('|');
            int len = recvDatas.Length;
            ReturnMessage = recvDatas[len - 1];
            ReturnCode = recvDatas[len - 2];
            if (string.Compare(recvDatas[len - 2].Trim(), "DSF3300") != 0)
            {
                ret = false;
            }
            else
        ...
        private bool UnpackJsonfix()
        {
            bool ret = false;

            if (string.Compare(RecvPackageJson.ReturnCode.Trim(), "00") != 0)
            {
                Log.Error("return json err:" + RecvPackageJson.returnMessage);
                ret = false;
            }
            else
                ret = true;
