...
namespace Landi.FrameWorks
{
    ...
    public abstract class PackageBase
    {
        ...
        protected int KeyLength = 8;//密钥长度
        ...
        /// <param name="DataOut">计算出的MAC值</param>
        /// <returns>true表示计算成功，否则表示失败</returns>
        protected virtual bool CalcMacByMackeySign(byte[] macBytes, byte[] MAC)
        {
            bool ret = false;
            ret = CalcMac_CBC_X99(this, macBytes, KeyManager.GetEnMacKey(mSectionName), KeyManager.GetDeMacKey(mSectionName), AlgorithmType.X99, MAC);
            //AppLog.Write("mac:" + Utility.bcd2str(KeyManager.GetDeMacKey(mSectionName), 16),AppLog.LogMessageType.Debug);
            return ret;
        }
        ...
        protected virtual bool CalcMacByPinkeySign(byte[] macBytes, byte[] MAC)
        {
            bool ret = false;

            ret = CalcMac_CBC_X99(this, macBytes, KeyManager.GetEnPinKey(mSectionName), KeyManager.GetDePinKey(mSectionName), AlgorithmType.X99, MAC);
            return ret;
        }
