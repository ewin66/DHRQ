...
{
    public class CardIssuer : HardwareBase<CardIssuer, CardIssuer.Status>, Landi.FrameWorks.IManagedHardware
    {
        #region CardIssuer.dll
        [DllImport("CardIssuer.dll")]
        protected static extern int Card_Init(string port_name, int bps, int capture);
        [DllImport("CardIssuer.dll")]
        protected static extern int Card_GetVersion(byte[] version);
        [DllImport("CardIssuer.dll")]
        protected static extern int Card_GetStatus(out int cardPos, out int boxNum);
        [DllImport("CardIssuer.dll")]
        protected static extern int Card_Pass(int passPos);
        [DllImport("CardIssuer.dll")]
        ...
        [DllImport("CardIssuer.dll")]
        protected static extern int Card_ReadAll(byte[] track1, byte[] track2, byte[] track3);
        [DllImport("CardIssuer.dll")]
        ...
        public static Status Pass(CardPassPos passType)
        {
            ...
            try
            {
                ...
                if (ret != Status.CARD_SUCC)
                {
                    AppLog.Write("[CardIssuer][Pass]" + ret.ToString(), AppLog.LogMessageType.Warn);
                }
                return ret;
            }
            catch (System.Exception e)
            {
                AppLog.Write("[CardIssuer][Pass]Error!\n" + e.ToString(), AppLog.LogMessageType.Error);
                return Status.CARD_FAIL;
            }
        }
