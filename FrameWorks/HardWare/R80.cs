...
namespace Landi.FrameWorks.HardWare
{
	...
	{
		[DllImport("R80.dll")]
		protected static extern int EA_mifare_sOpenport(string pcPort, int uiBps);
		[DllImport("R80.dll")]
		protected static extern int EA_mifare_sCloseport();
		[DllImport("R80.dll")]
		protected static extern int EA_mifare_sICActive(int iDelaytime, byte[] pucType, byte[] pucUIDLen, 
								   byte[] pucCardUID, byte[] pucATRLen, 
								   byte[] pucATRData);
		[DllImport("R80.dll")]
		protected static extern int EA_mifare_sICHalt(int iDelaytime);

		[DllImport("R80.dll")]
		protected static extern int EA_mifare_sICPowerup(int iDelaytime, byte[] pucPTL,
									byte[] pucATRLen, byte[] pucATRData);

		[DllImport("R80.dll")]
		protected static extern int EA_Alarm(int nTime);
